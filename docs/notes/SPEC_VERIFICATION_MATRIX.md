...
| 6.4 | Operator cockpit | v5.2 §5 | Single-command live system view | `scripts/operator_cockpit.py`. Parallel service health, agent table, blocker detection. | Cockpit renders live status. JSON snapshot to `state/logs/cockpit_snapshot.json`. | **LIVE** | Watchboard (sixth pass) | — |
