...
import { N as sanitizeUserFacingText, i as isMessagingToolDuplicate } from "./pi-embedded-helpers-WkKFkFQ7.js";
...
function normalizeReplyPayloadsForDelivery(payloads) {
	...
	for (const payload of payloads) {
		...
		const sanitizedText = sanitizeUserFacingText(parsed.text ?? "", { errorContext: Boolean(payload.isError) });
		const next = {
			...payload,
			text: sanitizedText ?? "",
