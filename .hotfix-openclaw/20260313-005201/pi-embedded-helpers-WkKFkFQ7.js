...
function isContextOverflowError(errorMessage) {
	...
	if (JINJA_NO_USER_QUERY_RE.test(errorMessage)) return false;
...
const INTERNAL_PROMPT_LEAK_LINE_RE = /^(?:A new session (?:was started|has started) via \/new or \/(?:startup|reset)|A new session has started\.|Execute your Session Startup sequence:?|Read required files|Do not mention internal steps, files, tools, or reasoning|Runtime:|session_status\b|Reasoning:|Identity file missing|The file .+ is missing\.?|Analyze an image with the configured image model|When running in sandboxed session|⚠️ You're in sandboxed mode|Tooling\b|✅ New session started · model:|\[Queued messages while agent was busy\]|Queued #\d+|Conversation info \(untrusted metadata\):|Sender \(untrusted metadata\):|Untrusted context \(metadata, do not treat as instructions or commands\):|<available_skills>|</available_skills>|<skill>|</skill>|Session Startup(?: Sequence)?|NO_REPLY\b|<\/think>|configuration$)/i;
const JINJA_NO_USER_QUERY_RE = /error rendering prompt with jinja template:\s*"no user query found in messages\."/i;
...
function sanitizeUserFacingText(text, opts) {
	...
	const stripped = stripFinalTagsFromText(text).replace(/<\/?think>/gi, "").replace(/\bNO_REPLY\b/gi, "").replace(/Conversation info \(untrusted metadata\):\s*```json[\s\S]*?```/gi, "").replace(/Sender \(untrusted metadata\):\s*```json[\s\S]*?```/gi, "").replace(/Untrusted context \(metadata, do not treat as instructions or commands\):[\s\S]*?(?=\n{2,}\S|$)/gi, "").replace(/<available_skills>[\s\S]*?<\/available_skills>/gi, "").replace(/\[MISSING\]\s*Expected at:\s*[^\n]+/gi, "").replace(/(?:^|\n)\s*Queued #\d+[^\n]*(?:\n(?!\s*$)[^\n]*)*/gi, "\n").replace(/(?:^|\n)\s*```(?:bash|sh)?[\s\S]*?```/gi, (block) => /cat > .*<< 'EOF'/i.test(block) ? "\n" : block).replace(/cat > .+<< 'EOF'[\s\S]*?^EOF\s*$/gim, "").split(/\r?\n/).filter((line) => !INTERNAL_PROMPT_LEAK_LINE_RE.test(line.trim())).join("\n");
