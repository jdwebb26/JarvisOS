...
def test_ralph_execution_contracts_and_reporting_surface_cleanly(tmp_path: Path):
    ...
    assert export_payload["execution_contract_summary"]["latest_backend_execution_result"]["request_kind"] == "ralph_consolidation"
...
def test_publish_readiness_recomputes_after_promotion_demoted_before_publish(tmp_path: Path):
    created = create_task_from_message(
        text="task: write a general note",
        user="tester",
        lane="tests",
        channel="tests",
        message_id="msg_demoted_readiness",
        root=tmp_path,
    )
    task_id = created["task_id"]
    artifact = write_text_artifact(
    ...
    assert invalidated_task.publish_readiness_status == "invalidated"
    assert invalidated_task.final_outcome == "candidate_invalidated"
    assert any(ref.startswith("demoted:") for ref in invalidated_task.blocked_dependency_refs)
