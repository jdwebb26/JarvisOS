...
def test_operator_checkpoint_action_pack_produces_commands(tmp_path: Path):
    ...
    assert "runtime/gateway/review_decision.py" in pack["pending_review_commands"][0]["commands"]["approve"]
    assert "runtime/gateway/approval_decision.py" in pack["pending_approval_commands"][0]["commands"]["approve"]
    assert "runtime/gateway/memory_decision.py" in pack["memory_decision_commands"][0]["commands"]["promote"]
