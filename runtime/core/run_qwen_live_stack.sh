...
while [ "$i" -le "$CYCLES" ]; do
  echo "[$(date -Is)] qwen live stack cycle $i/$CYCLES"
  python3 /home/rollan/.openclaw/workspace/jarvis-v5/runtime/core/qwen_live_recommender.py || true
  python3 /home/rollan/.openclaw/workspace/jarvis-v5/runtime/core/qwen_live_worker.py || true
  if [ "$i" -lt "$CYCLES" ]; then
    sleep "$SLEEP_SEC"
  fi
  i=$((i+1))
