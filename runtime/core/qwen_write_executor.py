...
def main() -> int:
    ap = argparse.ArgumentParser(description="Dry-run write executor for approved, allowlisted patches.")
        ...
        out = write_artifact(
            ...
            "\n".join(
                [
                    ...
                    f"- timestamp: {now_iso()}",
                    "- ready: false",
                    "- reason: no patch plan found",
                    "",
                ]
            ),
        )
        payload = {"ok": True, "ready": False, "reason": "no patch plan found", "artifact": str(out)}
    ...
    mode = str(approval.get("mode") or "dry_run")
    ...
    outcome = (
        "Dry-run write execution is ready. All approvals and allowlist checks passed. No code changes were applied."
        if ready and mode == "dry_run"
        else "Write execution is not ready. No code changes were applied."
    )
    ...
    out = write_artifact("write_execution", body)
    payload = {
        "ok": True,
        "ready": ready,
        "mode": mode,
