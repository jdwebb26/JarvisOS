...
AGENT_SKILL_ALLOWLIST: dict[str, tuple[str, ...]] = {
    ...
    "bowser": ("session-logs",),
    "cadence": ("sherpa-onnx-tts", "session-logs"),
    ...
    "vizor": ("session-logs", "model-usage"),
    "ict": ("session-logs", "model-usage"),
}
...
CANONICAL_AGENT_ROSTER: dict[str, dict[str, Any]] = {
    "jarvis": {
        ...
        "avoid": [
            ...
            "raw voice transcript handling — route to cadence instead",
            "voice session state management — cadence owns this",
        ],
    ...
    "cadence": {
        ...
        "role": "voice ingress / realtime conversation / intent routing",
        "status": "wired",
        "kind": "ingress",
        "responsibilities": [
            "receive and validate voice utterances",
            "classify intent: local_quick, jarvis_orchestration, hal_task, kitt_quant, scout_research, browser_action, approval_confirmation, voice_subsystem",
            "maintain lightweight voice session state",
            "route to appropriate specialist without burdening Jarvis",
            "surface TTS responses for voice-native interactions",
        ],
        "avoid": [
            "deep code implementation",
            "final approval decisions",
            "browser automation execution (delegate to bowser)",
            "long-running research (delegate to scout/hermes)",
        ],
        "task_classes": ["voice", "general", "approval"],
        "routing_intent": {
            "preferred_model": "Qwen3.5-9B",
            "fallbacks": ["Qwen3.5-35B"],
            "primary_backend": "cadence_ingress",
        },
        "allowed_tool_categories": ["voice", "coordination", "general"],
        "denied_tool_categories": ["browser", "research", "maintenance", "engineering", "review", "creative"],
        "denied_tool_name_tokens": ["clawhub", "weather", "exec", "shell", "bash", "patch"],
        "preferred_skill_task_classes": ["voice", "general", "approval"],
        "preferred_skill_backends": ["cadence_ingress"],
        "preferred_channels": ["voice", "jarvis"],
    },
    "muse": {
        "agent_id": "muse",
        "display_name": "Muse",
    ...
    "vizor": {
        "agent_id": "vizor",
        "display_name": "Vizor",
        "role": "visual quant analyst / chart reader / dual-model vision",
        "status": "wired",
        "kind": "specialist",
        "responsibilities": [
            "trading chart analysis (any platform)",
            "ICT/SMC pattern identification via vision models",
            "key level identification with precise prices",
            "setup quality rating (A+ through C)",
            "dual-model comparison (EyeNet-Personal vs EyeNet-Swarm)",
            "visual market data interpretation (heatmaps, DOM, footprint, order flow)",
        ],
        "avoid": [
            "trade execution (Executor's job)",
            "live data pulls (Kitt's job)",
            "modifying personal training data directly",
            "general operator chat (Jarvis's job)",
            "text-only methodology questions (ICT's job)",
        ],
        "task_classes": ["quant", "general"],
        "routing_intent": {
            "preferred_model": "vizor-personal",
            "fallbacks": ["vizor-swarm", "Qwen3.5-35B"],
            "primary_backend": "openai_executor",
        },
        "allowed_tool_categories": ["coordination", "repo_read", "general"],
        "denied_tool_categories": ["browser", "engineering", "maintenance", "voice", "creative"],
        "denied_tool_name_tokens": ["exec", "shell", "bash", "write", "edit", "patch", "clawhub", "weather"],
        "preferred_skill_task_classes": ["quant"],
        "preferred_skill_backends": ["openai_executor"],
        "preferred_channels": ["vizor"],
    },
    "ict": {
        "agent_id": "ict",
        "display_name": "ICT",
        "role": "ICT methodology expert / multimodal knowledge encyclopedia",
        "status": "wired",
        "kind": "specialist",
        "responsibilities": [
            "definitive authority on Inner Circle Trader methodology",
            "concept validation and precise definitions",
            "setup validation against ICT methodology",
            "killzone timing and session analysis",
            "multimodal chart + transcript understanding",
            "cross-reference related ICT concepts",
        ],
        "avoid": [
            "trade execution or signals",
            "third-party interpretations (primary source only)",
            "live data pulls (Kitt's job)",
            "visual-only analysis without methodology context (Vizor's job)",
            "general operator chat (Jarvis's job)",
        ],
        "task_classes": ["quant", "general"],
        "routing_intent": {
...
DELEGATION_WIRING = [
    ...
    {"from_agent": "jarvis", "to_agent": "cadence", "task_classes": ["voice", "general"], "status": "wired", "source": "runtime/voice/cadence_ingress.py", "delegation_method": "external_adapter", "note": "Call route_cadence_utterance() for all voice utterances. Cadence owns session state and intent routing; Jarvis should not handle raw voice transcripts."},
    {"from_agent": "cadence", "to_agent": "jarvis", "task_classes": ["general", "approval", "output"], "status": "wired", "source": "runtime/voice/cadence_ingress.py", "delegation_method": "message_tool", "note": "Cadence delegates unclassified or orchestration intents to Jarvis via notification/message."},
    {"from_agent": "cadence", "to_agent": "hal", "task_classes": ["code", "deploy"], "status": "wired", "source": "runtime/voice/cadence_ingress.py", "delegation_method": "external_adapter", "note": "Cadence creates code tasks via create_task_from_message() when utterance is classified as hal_task."},
    {"from_agent": "cadence", "to_agent": "kitt", "task_classes": ["quant", "research"], "status": "wired", "source": "runtime/voice/cadence_ingress.py", "delegation_method": "external_adapter", "note": "Cadence creates quant tasks via create_task_from_message() when utterance is classified as kitt_quant."},
    {"from_agent": "cadence", "to_agent": "scout", "task_classes": ["research", "docs"], "status": "wired", "source": "runtime/voice/cadence_ingress.py", "delegation_method": "external_adapter", "note": "Cadence creates research tasks via create_task_from_message() when utterance is classified as scout_research."},
    {"from_agent": "cadence", "to_agent": "bowser", "task_classes": ["browser"], "status": "wired", "source": "runtime/voice/cadence_ingress.py", "delegation_method": "external_adapter", "note": "Cadence delegates browser_action intents to bowser via run_browser_task()."},
]
